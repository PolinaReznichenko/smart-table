export function initFiltering(elements) {
  // Функция заполнения выпадающих списков опциями из справочников с сервера (продавцов, покупателей, ...)
  ...
  // Функция, которая формирует и добавляет часть запроса к серверу, отвечающую за фильтрацию (для непустых полей)
  const applyFiltering = (query, state, action) => {
    if (action && action.name === "clear") {  // обработка очистки поля
